...
interface BottomNavProps {
  activeTab: 'home' | 'catalog' | 'chat' | 'rewards' | 'profile';
  onTabChange: (tab: 'home' | 'catalog' | 'chat' | 'rewards' | 'profile') => void;
  onCartClick: () => void;
}

export default function BottomNav({ activeTab, onTabChange, onCartClick }: BottomNavProps) {
  const { cart } = useCart();
  ...
  const tabs = [
    ...
    },
    {
      ...
      icon: (active: boolean) => (
        <svg className="w-6 h-6" fill={active ? 'currentColor' : 'none'} stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={active ? 0 : 1.5} d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
        </svg>
      ),
    },
    {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 safe-bottom z-40">
      <div className="flex items-center justify-around px-2 py-2">
        {tabs.map((tab) => {
          ...
          return (
            ...
            >
              ...
              <span className={`text-[10px] mt-1 font-medium ${isActive ? 'text-[#00b37e]' : ''}`}>
                {tab.label}
              </span>
            </button>
          );
        ...
        <button
          onClick={onCartClick}
          className="absolute -top-6 right-4 w-14 h-14 bg-[#00b37e] rounded-full shadow-lg flex items-center justify-center text-white hover:bg-[#00996b] active:scale-95 transition-all"
        >
          <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" />
          </svg>
          {cart.itemCount > 0 && (
            <span className="absolute -top-1 -right-1 w-6 h-6 bg-red-500 text-white text-xs font-bold rounded-full flex items-center justify-center cart-badge-animate">
